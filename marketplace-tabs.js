document.addEventListener("DOMContentLoaded", () => {
  ...
  function createAlterCard(alter) {
    ...
    card.className =
      "bg-secondary bg-opacity-30 rounded-lg overflow-hidden product-card";
    card.innerHTML = `
          <div class="relative">
              <img src="${alter.avatar_url}" alt="${
      alter.name
          ...
          <div class="p-4">
              ...
              <div class="flex items-center mb-4">
                  <div class="w-6 h-6 rounded-full overflow-hidden mr-2">
                      <img src="${
                        alter.creator_avatar || "/placeholder.svg"
                      }" alt="Creator" class="w-full h-full object-cover" />
                  </div>
                  <span class="text-sm text-gray-400">by <span class="text-primary">${
                    alter.creator_name || "Unknown"
                  }</span></span>
              ...
              <div class="flex items-center justify-between">
                  ...
                  ${
                    alter.is_owner
                      ? `<button class="delete-button ml-2" data-alter-id="${alter.id}"><i class="fas fa-trash"></i></button>`
                      : ""
                  }
              </div>
          </div>
    ...
    if (talkButton) {
      talkButton.addEventListener("click", () => {
        ...
        window.location.href = "/chat-alter"; // Redirect to chat page
      });
    }
  ...
  async function deleteAlter(alterId) {
    ...
    try {
      ...
      const card = document
        .querySelector(`[data-alter-id="${alterId}"]`)
        .closest(".alter-card");
      card.remove();
